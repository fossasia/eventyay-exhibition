...
class LeadCreateView(views.APIView):
    def get_allowed_attendee_data(self, order_position, settings, exhibitor):
        # Notes and tags are always available on the lead record itself; voucher
        # (ticket/attendee) fields are only exposed when allow_voucher_access is on.
...
class LeadRetrieveView(views.APIView):
    def get(self, request, *args, **kwargs):
        ...
        if not exhibitor.allow_lead_access:
            return Response(
                {"success": False, "error": "Lead data access is not allowed for this exhibitor"},
                status=status.HTTP_403_FORBIDDEN,
            )
